...
jsPsych.plugins['multiple-slider-adap'] = (function() {

    var plugin = {};
  
    plugin.info = {
      name: 'multiple-slider-adap',
      description: '',
    parameters: {
      questions: {
      ...
      disable_all_sliders: {
        type: jsPsych.plugins.parameterType.BOOL, 
        pretty_name: 'Disable all sliders',
        default: false,
        description: 'If true, all sliders are disabled to be changed.'
      },
      t_before_button: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'time before button press',
        default: 0,
        description: 'time in which the continue button cannot be pressed'
      },
      slider_width: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // if all sliders should be disabled 
    if (trial.disable_all_sliders) {
      //document.getElementsByClassName('jspsych-html-slider-response-response').disabled = true;
      for (var i = 0;i<trial.questions.length;i++){
          document.getElementById('jspsych-html-slider-response-response-'+i).disabled = true;
      };
    }

    // time that button press is disabled
    if (trial.t_before_button != 0){
      ...
      }, trial.t_before_button);
    }

    var slider_changed_idx = [];
    ...
    if (trial.require_movement) {
      ...
      // check whether one slider has been changed
      function check_reponses() {
        var all_sliders = document.querySelectorAll('.jspsych-html-slider-response-response');
        var one_slider_changed = false;
        for (var i=0; i<all_sliders.length; i++) {
            if (all_sliders[i].classList.contains("changed")) {
                  one_slider_changed = true;
                  slider_changed_idx = i; // get the slider that was changed

                  // make sure to just log the first click (don't override changeTime if it is logged once)
                   if (changeTime == 0){
        ...
        if (one_slider_changed) {
          // if one slider has been changed then enable the next button
          document.getElementById('jspsych-multiple-slider-next').disabled = false;
          // ...and disable the other sliders
            for (var i = 0; i<all_sliders.length; i++){
                if (i != slider_changed_idx){
                    document.getElementById('jspsych-html-slider-response-response-'+i).disabled = true;
                }
            }
        }
      ...
      all_sliders.forEach(function(slider) {
          slider.addEventListener('change', function() {
            slider.classList.add("changed"); // record the fact that this slider has been clicked
    ...
    display_element.querySelector('#jspsych-multiple-slider-form').addEventListener('submit', function(e){
      ...
      var response_time_change = changeTime - startTime;
      ...
      // save data
      var trial_data = {
        "rt": response_time_change,
        "rt_btn": response_time,
        "response_slider": label_sel,
